...
import { bigIslandBeaches, bigIslandTrails } from "../data/bigIslandData";
import { BreadcrumbSchema } from "../components/SchemaMarkup";
import gsap from "gsap";
import {
  Umbrella, Mountain, Eye, Waves, MapPin, Shield,
  Bath, Droplets, Car, ArrowLeft, Mountain as VolcanoIcon
} from "lucide-react";

const beachImages: Record<string, string> = {
  "Hapuna Beach State Recreation Area": "/images/bigisland-beaches/hapuna.jpg",
  "Papakolea Beach (Green Sand Beach)": "/images/bigisland-beaches/papakolea.jpg",
};
...
const breadcrumbItems = [
  ...
  { name: "Big Island", url: "https://www.beachesnhikes.com/bigisland" },
];
...
export default function BigIslandPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      gsap.fromTo(".bi-fade-in", { opacity: 0, y: 20 }, { opacity: 1, y: 0, duration: 0.6, stagger: 0.05, ease: "power2.out" });
    }, heroRef);
    return () => ctx.revert();
  }, [activeTab]);

  const beachRegions = ["All", "Kohala Coast", "Kona", "Ka'u", "Hilo", "Puna", "Hamakua"];
  const trailRegions = ["All", "Volcanoes National Park", "Kona", "Ka'u", "Kohala", "Hilo", "Hamakua", "Saddle Road"];

  const filteredBeaches = beachFilter === "All" ? bigIslandBeaches : bigIslandBeaches.filter(b => b.region === beachFilter);
  const filteredTrails = trailFilter === "All" ? bigIslandTrails : bigIslandTrails.filter(t => t.region === trailFilter);

  return (
    <div className="min-h-screen bg-warm-white" ref={heroRef}>
      <Helmet>
        <title>Big Island Beaches & Hiking Trails | 16 Beaches & 12 Trails | Beaches & Hikes</title>
        <meta name="description" content="Discover the Big Island's volcanic beaches and epic trails — from green sand to black lava. 16 beaches, 12 trails, and insider tips for Hawaii Island." />
        <meta property="og:title" content="Big Island Beaches & Hiking Trails Guide | Beaches & Hikes" />
        <meta property="og:description" content="16 beaches and 12 trails across volcanic landscapes — from green sand to active lava flows. Your complete Big Island guide." />
        <meta property="og:image" content="https://www.beachesnhikes.com/images/bigisland-hero.jpg" />
        <meta property="og:type" content="website" />
        <meta property="og:url" content="https://www.beachesnhikes.com/bigisland" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content="Big Island Beaches & Hiking Trails Guide" />
        <meta name="twitter:description" content="16 beaches and 12 trails across volcanic landscapes." />
        <meta name="twitter:image" content="https://www.beachesnhikes.com/images/bigisland-hero.jpg" />
      </Helmet>
      ...
      <div className="relative h-[45vh] bg-deep-forest overflow-hidden">
        <img
          src="/images/bigisland-hero.jpg"
          alt="Big Island Hawaii volcanic landscape with lava and ocean"
        ...
        <div className="absolute bottom-0 left-0 right-0 z-10" style={{ padding: "0 clamp(24px, 4vw, 64px) 40px" }}>
          <div className="max-w-[1400px] mx-auto">
            <span className="font-body text-[12px] tracking-[0.15em] uppercase text-warm-white/70">Hawaii Island</span>
            <h1 className="font-display text-[48px] md:text-[64px] text-warm-white leading-[1.05] mt-2">The Big Island</h1>
            <p className="font-body text-[16px] text-warm-white/80 mt-3 max-w-[600px] leading-relaxed">
              Home to active volcanoes, green and black sand beaches, and the tallest mountain on Earth. {bigIslandBeaches.length} beaches &amp; {bigIslandTrails.length} hiking trails.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-sand/30 border-b border-deep-forest/5">
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 py-4 flex items-center gap-6 overflow-x-auto">
          ...
          <span className="font-body text-[14px] text-deep-forest font-medium whitespace-nowrap border-b-2 border-ocean pb-4 -mb-4">Big Island</span>
        </div>
      </div>

      {/* Info Banner */}
      <div className="max-w-[1400px] mx-auto px-6 md:px-10 pt-10">
        <div className="bg-sand/30 border border-deep-forest/5 rounded-xl p-4 mb-6">
          <p className="font-body text-[13px] text-deep-forest/70 leading-relaxed">
            <strong className="text-deep-forest">The Big Island has it all:</strong> White sand beaches on the Kohala and Kona coasts, jet-black volcanic sand in Ka&apos;u and Puna, and the rare green sand at Papakolea — one of only four in the world. The Kona/Kohala side is sunny and dry with the best swimming; the Hilo side is lush and wet with unique snorkeling spots.
          </p>
        </div>
        ...
        <div className="flex gap-2 mb-8">
          <button onClick={() => setActiveTab("beaches")} className={`px-5 py-2.5 rounded-full font-body text-[14px] transition-all duration-300 inline-flex items-center gap-2 ${activeTab === "beaches" ? "bg-deep-forest text-sand" : "border border-deep-forest/20 text-deep-forest hover:bg-deep-forest/5"}`}>
            <Umbrella size={15} /> Beaches ({bigIslandBeaches.length})
          </button>
          <button onClick={() => setActiveTab("trails")} className={`px-5 py-2.5 rounded-full font-body text-[14px] transition-all duration-300 inline-flex items-center gap-2 ${activeTab === "trails" ? "bg-deep-forest text-sand" : "border border-deep-forest/20 text-deep-forest hover:bg-deep-forest/5"}`}>
            <Mountain size={15} /> Hiking Trails ({bigIslandTrails.length})
          </button>
        </div>
      </div>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 pb-20">
          <div className="flex flex-wrap gap-2 mb-8">
            {beachRegions.map(region => (
              <button key={region} onClick={() => setBeachFilter(region)} className={`px-4 py-1.5 rounded-full font-body text-[13px] transition-all duration-300 ${beachFilter === region ? "bg-ocean text-white" : "border border-deep-forest/10 text-deep-forest/70 hover:border-ocean/40"}`}>{region}</button>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {filteredBeaches.map((beach) => {
              ...
              return (
                <div key={beach.name} className="bi-fade-in bg-white rounded-2xl overflow-hidden border border-deep-forest/5 hover:shadow-lg hover:shadow-deep-forest/5 transition-all duration-300 group">
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-10 pb-20">
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {filteredTrails.map((trail) => (
              <div key={trail.name} className="bi-fade-in bg-white rounded-2xl overflow-hidden border border-deep-forest/5 hover:shadow-lg hover:shadow-deep-forest/5 transition-all duration-300">
                <div className="relative h-[160px] bg-gradient-to-br from-volcano/10 to-sand/40 flex items-center justify-center">
                  <VolcanoIcon className="w-12 h-12 text-orange-800/30" strokeWidth={1} />
                  <div className="absolute top-3 right-3">
